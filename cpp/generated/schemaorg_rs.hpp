...
class NativeSchemaorgRs : public jsi::HostObject {
private:
...
protected:
  ...
  jsi::Value cpp_uniffi_internal_fn_func_ffi__arraybuffer_to_string(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);
  jsi::Value cpp_ffi_schemaorg_rs_uniffi_contract_version(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);
...
public:
  NativeSchemaorgRs(
      jsi::Runtime &rt,
      std::shared_ptr<uniffi_runtime::UniffiCallInvoker> callInvoker);
  virtual ~NativeSchemaorgRs();

  /**
   * The entry point into the crate.
   *
   * React Native must call `NativeSchemaorgRs.registerModule(rt, callInvoker)`
   ...
   * be the case.
   *
   * Clients should call `NativeSchemaorgRs.unregisterModule(rt)` after final
   * use where possible.
   */
